...
.Input("const_starts: int32")
.Input("const_ends: int32")
.Input("const_labels: int32")
.Input("ner_starts: int32")
.Input("ner_ends: int32")
.Input("ner_labels: int32")
.Output("starts: int32")
.Output("ends: int32")
.Output("output_const_labels: int32")
.Output("output_ner_labels: int32");
...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    TTypes<int32>::ConstVec const_starts = context->input(1).vec<int32>();
    TTypes<int32>::ConstVec const_ends = context->input(2).vec<int32>();
    TTypes<int32>::ConstVec const_labels = context->input(3).vec<int32>();
    TTypes<int32>::ConstVec ner_starts = context->input(4).vec<int32>();
    TTypes<int32>::ConstVec ner_ends = context->input(5).vec<int32>();
    TTypes<int32>::ConstVec ner_labels = context->input(6).vec<int32>();

    int num_words = sentence_indices.dimension(0);
    int num_const_spans = const_labels.dimension(0);
    int num_ner_spans = ner_labels.dimension(0);

    CHECK_EQ(num_const_spans, const_starts.dimension(0));
    CHECK_EQ(num_const_spans, const_ends.dimension(0));
    CHECK_EQ(num_ner_spans, ner_starts.dimension(0));
    CHECK_EQ(num_ner_spans, ner_ends.dimension(0));

    std::vector<std::pair<int, int>> spans;
    for (int i = 0; i < num_words; ++i) {
      for (int j = i; j < num_words && (j - i) < _max_width; ++j) {
    ...
    Tensor* output_const_labels_tensor = nullptr;
    Tensor* output_ner_labels_tensor = nullptr;
    ...
    OP_REQUIRES_OK(context, context->allocate_output(2, outputs_shape, &output_const_labels_tensor));
    OP_REQUIRES_OK(context, context->allocate_output(3, outputs_shape, &output_ner_labels_tensor));
    TTypes<int32>::Vec starts = starts_tensor->vec<int32>();
    TTypes<int32>::Vec ends = ends_tensor->vec<int32>();
    TTypes<int32>::Vec output_const_labels = output_const_labels_tensor->vec<int32>();
    TTypes<int32>::Vec output_ner_labels = output_ner_labels_tensor->vec<int32>();

    std::map<std::pair<int, int>, int> const_spans;
    for (int i = 0; i < num_const_spans; ++i) {
      const_spans[std::pair<int, int>(const_starts(i), const_ends(i))] = const_labels(i);
    }
    std::map<std::pair<int, int>, int> ner_spans;
    for (int i = 0; i < num_ner_spans; ++i) {
      ner_spans[std::pair<int, int>(ner_starts(i), ner_ends(i))] = ner_labels(i);
    }
    ...
    for (int i = 0; i < spans.size(); ++i) {
      ...
      auto const_spans_iter = const_spans.find(std::pair<int, int>(span.first, span.second));
      if (const_spans_iter != const_spans.end()) {
        output_const_labels(i) = const_spans_iter->second;
      } else {
        output_const_labels(i) = 0;
      }

      auto ner_spans_iter = ner_spans.find(std::pair<int, int>(span.first, span.second));
      if (ner_spans_iter != ner_spans.end()) {
        output_ner_labels(i) = ner_spans_iter->second;
      } else {
        output_ner_labels(i) = 0;
      }
    }
  }
